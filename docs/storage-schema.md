...
現在の`.fukushu`バックアップは設定、問題集、問題、学習状態、レビュー履歴、読み込み操作の履歴、元APKG、テンプレートメタデータ、メディアを含みます。旧JSONバックアップversion 1〜3も復元できます。新形式の構造と大容量への対応は[バックアップ](backup-support.md)を参照してください。復元時はValibot、メディアのサイズ・SHA-256、内部参照、HTML/CSSの検査を行います。
